...
export default function ClaimsQueue() {
  ...
  return (
    <div className="p-8" data-testid="claims-queue">
      <div className="mb-8">
        <div className="text-xs text-primary-700 uppercase tracking-widest font-semibold">Queue</div>
        <h1 className="font-display text-4xl font-semibold tracking-tight mt-1">Claims Queue</h1>
      </div>
          ...
          <tbody>
            {filtered.map((c) => (
              <tr key={c.id} data-testid={`admin-claim-${c.claim_number}`} className="border-t border-gray-100">
                <td className="p-4 font-mono text-xs">{c.claim_number}</td>
                <td className="p-4">{c.incident_type}</td>
                <td className="p-4 font-medium">${c.amount_claimed.toFixed(2)}</td>
                <td className="p-4">
                ...
                <td className="p-4">
                  <span className={`text-xs font-semibold px-2 py-1 rounded-full capitalize ${BADGE[c.status]}`}>
                    {c.status.replace("_", " ")}
                  </span>
                </td>
                <td className="p-4">
